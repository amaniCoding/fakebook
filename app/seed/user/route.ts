...
async function seedPostComments() {
  ...
  const comments = await Promise.all(
    posts.map((post) => {
      const randomCommentCount = Math.floor(Math.random() * 51) + 50;
    ...
    posts.map((post) => {
      return Promise.all(
        Array.from(Array(50).keys()).map(() => {
          const randomUserIndex = Math.floor(Math.random() * 20);
          const randomUser = users[randomUserIndex];
          return client.sql`INSERT INTO ureactions (postid, userid, reactiontype) VALUES (${post.postid}, ${randomUser.userid}, 'care') ON CONFLICT (reactionid) DO NOTHING`;
        })
      );
    })
  );
